...
Review Writer 是一个面向 Windows 的本地桌面应用。它把调研需求、综述协议、文献检索、全文管理、结构化精读、报告生成和引用核验保存在同一个可恢复项目中。

当前版本：`0.7.0`

## 为什么做这个应用

文献调研常散落在数据库网页、PDF、笔记软件、参考文献管理器和报告文档中。研究者很容易丢失筛选理由、原文位置，或忘记某个结论来自摘要还是全文。

大模型增加了另一个风险：语言流畅的段落可能超出已有材料。Review Writer 为论文、文档资产、证据块和报告论断保留来源定位。应用在模型调用前显示材料等级、Token 数量与费用估算，并把“生成报告”和“核验报告”分成两个任务。

这个项目希望提供一套可检查、可恢复的本地工作流。它不会代替研究者判断，也不会把自动生成的文字当成已经核验的事实。
